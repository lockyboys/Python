# [main_organizer.py] - 전체 시스템 통합 실행 파일 (v16 - Expert Mode)
import os
import sys
import config
import utils
...
def run_total_organization(path_str):
    try:
        ...
        # 1. 시스템 진단 및 보고서 생성 (로그 자동 기록)
        utils.get_system_status()
        
        print(f"\n📂 대상 폴더: {path_str}")
        if config.UNPACK_ALL:
            print("⚠️ [해체 모드] 모든 하위 폴더의 파일을 루트로 모읍니다.")
        ...
        for item in all_files:
            try:
                ...
                # 이미 분류된 폴더 제외 (UNPACK_ALL이 아닐 때만)
                ...
                utils.log_message(f"파일 처리 오류 ({item.name}): {e}", "ERROR")
        ...
        print(f"📝 상세 진단 및 오류 로그는 '{config.LOG_FILE_PATH}'에 저장되었습니다.")
        
    except Exception as e:
        utils.log_message(f"메인 프로세스 치명적 오류: {e}", "CRITICAL", True)
